...
def main():
  ...
  mach = cal_machnumber(gamma, area_ratio)
  print('Area ratio and gamma:',area_ratio, gamma)
  print('Mach number',mach)


  # Series 
  flag_fileseries = True
  ...
  # --Set area ratio range
  area_ratio_series = np.linspace(1.0, 100, 100)
  ...
  pres_ratio_series = []
  dens_ratio_series = []
  for n in range(0,len(area_ratio_series)):
    area_ratio_tmp = area_ratio_series[n]
    mach_tmp = cal_machnumber(gamma, area_ratio_tmp)
  ...
  header = 'Variables = AnebyAth, mach, temperature_ratio(TbyT0),pressure_ratio(pbyp0),density_ratio(rhobyrho0) \n zone t='+str(casename)+' i= '+str(len(mach_series))+' f=point'
  output_data = np.c_[ area_ratio_series,
                       mach_series_ndarray,
                       temp_ratio_ndarray,
                       pres_ratio_ndarray,
                       dens_ratio_ndarray
